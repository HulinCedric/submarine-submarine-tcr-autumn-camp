...
public class SubmarineShould
{
    ...
    public void Move_X_on_position_like_in_forward_X_command(string command, int expectedPosition)
    {
        var submarine = Submarine().Build();
    ...
    {
        var submarine = Submarine().WithAim(10).Build();

        submarine.ExecuteCommand("up 5");

        submarine.Should().BeEquivalentTo(Submarine().WithAim(5).Build());
    }
